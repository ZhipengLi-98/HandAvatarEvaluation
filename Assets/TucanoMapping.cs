...
public class TucanoMapping : MonoBehaviour
{
    private string mappingFile = "./Assets/tucano_mapping.txt";
    private string poseFile = "tucano_hand_pose.txt";
        ...
        if (flag)
        {
            foreach (KeyValuePair<GameObject, GameObject> pair in mapping)
            {
                if (pair.Value.transform.name.Contains("Forearm"))
                {
                    ...
                    pair.Key.transform.rotation = Quaternion.Euler(0, temp.eulerAngles.y, 180 + temp.eulerAngles.z);
                }
                else if (pair.Value.transform.name.Contains("Thumb"))
                {
                    ...
                    pair.Key.transform.localRotation = Quaternion.Euler(30 + 2 * temp.eulerAngles.z, 0, 0) * initial;
                }
                else if (pair.Value.transform.name.Contains("Pinky"))
                {
                    Quaternion temp = pair.Value.transform.localRotation * Quaternion.Inverse(initialHandRotations[pair.Value.transform.name]);
                    Quaternion initial = initialRotations[pair.Key.transform.name];
                    pair.Key.transform.localRotation = Quaternion.Euler(-30 - 2 * temp.eulerAngles.z, 0, 0) * initial;
