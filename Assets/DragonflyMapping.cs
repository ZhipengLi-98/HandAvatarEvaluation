...
public class DragonflyMapping : MonoBehaviour
{
    private string mapping_file = "./Assets/dragonfly_mapping.txt";
    ...
    private string poseFile = "dragonfly_hand_pose.txt";
        ...
        if (flag)
        {
            foreach (KeyValuePair<GameObject, GameObject> pair in mapping)
            {
                if (pair.Key.transform.name.Contains("trithemis_Thorax1_bone"))
                {
                    ...
                    pair.Key.transform.rotation = Quaternion.Euler(0, temp.eulerAngles.y, 0) * Quaternion.Euler(0, 120, 0);
                }
